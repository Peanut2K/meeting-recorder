...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Signed URL for the recording (bucket is private). Try mp3 first (new), fall back to webm (legacy).
  let signed = (await admin.storage.from('meeting-audio').createSignedUrl(`${meeting.team_id}/${id}.mp3`, 3600)).data
  if (!signed) {
    signed = (await admin.storage.from('meeting-audio').createSignedUrl(`${meeting.team_id}/${id}.webm`, 3600)).data
  }
